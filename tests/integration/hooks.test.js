 ...
 * フック スクリプトの統合テスト
 *
 * 適切な入出力処理を伴う実運用に近いシナリオでフックの挙動をテストする。
 *
 * 実行: node tests/integration/hooks.test.js
 */
...
/**
 * Claude Code の入力を模擬してフック スクリプトを実行する
 * @param {string} scriptPath - フック スクリプトへのパス
 * @param {object} input - フック入力オブジェクト (JSON 文字列化される)
...
function runHookWithInput(scriptPath, input = {}, env = {}, timeoutMs = 10000) {
  return new Promise((resolve, reject) => {
    ...
    // EPIPE エラーを無視する (書き込み完了前にプロセスが終了する可能性があるため)
    ...
    // stdin に JSON 入力を送る (Claude Code のフック呼び出しを模擬する)
...
/**
 * hooks.json にあるようなインライン hook コマンドを実行する
...
async function runTests() {
  ...
  if (await asyncTest('hooks parse valid tool_input correctly', async () => {
    // 有効入力で console.log の警告フックをテストする
  ...
  if (await asyncTest('hooks output messages to stderr (not stdout)', async () => {
    const result = await runHookWithInput(path.join(scriptsDir, 'session-start.js'), {});
    // session-start は stderr に情報を書き出すべき
  ...
  if (await asyncTest('evaluate-session processes transcript with sufficient messages', async () => {
    ...
    // 15 件のユーザー メッセージを含む transcript を作成する
